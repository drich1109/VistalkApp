        ...
        {userDetails && (
          <View className="items-center mt-24">
            {fileUrl ? (
              <Image
                source={{ uri: fileUrl }}
                className="w-40 h-40 rounded-full mb-5 border-2 border-white"
              />
            ) : (
            ...
            <Text className="text-white text-2xl font-bold mb-2">{userDetails.name}</Text>
            <Text className="text-white text-lg">{userDetails.email}</Text>
          </View>
        )}
      </ImageBackground>
    </SafeAreaView>
