<script lang="ts">
    ...
    function selectChoice(choice: Content) {
        if (activeSide !== null && activeIndex !== null) {
            if (activeSide === 'left') {
                leftQueries[activeIndex] = choice.englishTranslation;
                searchQueries[activeIndex] = choice.englishTranslation;
            } else {
                rightQueries[activeIndex] = choice.englishTranslation;
            }
            selectedChoices[activeIndex] = choice;
        }
    }
    ...
    function handleInputChange(index: number, side: 'left' | 'right', event: Event) {
        ...
        searchQueries[index] = target.value;
        if (target.value === '') {
            selectedChoices[index] = undefined;
        }
    }

    function setActiveSide(side: 'left' | 'right', index: number) {
        activeSide = side;
        activeIndex = index;
    }
                ...
                <div class="mt-4 relative flex items-center">
                    ...
                    <div class="relative w-full flex items-center space-x-2">
                        <input
                            ...
                            on:focus={() => setActiveSide('left', i-1)}
                        ...
                        <input
                            ...
                            on:focus={() => setActiveSide('right', i-1)}
                            class="block w-1/2 px-3 py-2 text-gray placeholder-gray bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40"
                        />
                    </div>
