<script lang="ts">
    ...
    import { getQuestionTypes } from './repo';
...
{#if showModal}
    <AddQuestionPopup {languageId} {questionTypes} {unitId} {showModal} on:close={closeModal}></AddQuestionPopup>
{/if}
<div class="flex justify-between items-center mt-1 bg-white rounded-xl py-4 px-4 shadow-lg">
    ...
    <div class="flex gap-4">
        ...
        <button on:click={openModal} class="flex items-center font-['Helvetica'] bg-[#99BC85] text-white py-2 px-3 rounded-xl text-sm shadow-lg hover:bg-[#BFD8AF] transform hover:scale-110 transition-transform duration-300">
    ...
    <table class="bg-white w-full shadow-lg rounded-xl">
        <thead class="font-['Cambria'] bg-[#99BC85] text-white  text-center">
            <tr class="first:rounded-t-xl last:rounded-b-xl">
                <th class="px-4 py-2 first:rounded-tl-xl last:rounded-tr-xl">Word</th>
